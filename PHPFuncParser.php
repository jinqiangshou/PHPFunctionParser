...
class PHPFuncParser
{
    ...
    private function update_last_line($token){
        ...
        if(is_array($this->prev_token)){
            if($this->prev_token[0] == T_WHITESPACE){
    ...
    public function process()
    {
        ...
        if($this->left_class_brace_cnt != 0 || $this->left_brace_cnt != 0 || $this->left_func_method_brace_cnt != 0){
    ...
    private function should_change_in_method_to_in_class($token) {
        if (!$this->in_php || $this->state != self::STATE_IN_METHOD || is_array($token)) {
            return false;
        }
        if (is_string($token) && $token == "}" && $this->left_func_method_brace_cnt == 1 &&
    ...
    private function change_in_method_to_in_class($token) {
        if($this->state != self::STATE_IN_METHOD){
            return;
        }
        ...
        if (is_array($token)) {
            switch ($token[0]) {
                ...
                case T_STRING: //函数名称
                    ...
                    if($this->meet_to_in_func_method == 1) {
            ...
            switch ($token) {
                case "{":
                    ...
                    break;
                case "(":
                    if ($this->meet_to_in_func_method == 2) {
                        $this->meet_to_in_func_method = 3;
                    }
                    break;
                case ")":
                    if ($this->meet_to_in_func_method == 3) {
    ...
    private function change_meet_method_to_in_class($token){
        if ($this->state != self::STATE_MEET_METHOD) {
            return;
        }
        $this->state = self::STATE_IN_CLASS;
        if($token == "{") {
    ...
    private function should_change_meet_method_to_in_class($token){
        if (!$this->in_php || $this->state != self::STATE_MEET_METHOD || is_array($token)) {
            return false;
        }
    ...
    private function should_change_meet_method_to_in_method($token) {
        ...
        if (is_string($token) && $token == "{" && $this->meet_to_in_func_method == 4) {
            return true;
        }
        return false;
    }
    ...
    private function change_in_class_to_init($token) {
        if($this->state != self::STATE_IN_CLASS){
            return;
        }
        $this->state = self::STATE_INIT;
        $this->left_brace_cnt --;
    ...
    private function should_change_in_class_to_init($token) {
        if(!$this->in_php || $this->state != self::STATE_IN_CLASS || is_array($token)){
            return false;
        }
        if(is_string($token) && $token == "}" && $this->left_class_brace_cnt == 1 &&
            $this->left_func_method_brace_cnt == 0 && $this->left_brace_cnt == 1){
            return true;
        }
        return false;
    }
    ...
    private function should_change_in_class_to_meet_method($token) {
        if(!$this->in_php || $this->state != self::STATE_IN_CLASS || !is_array($token) || $token[0] != T_FUNCTION){
            return false;
        }
        if($this->class_name == "" || $this->left_class_brace_cnt < 1 || $this->left_func_method_brace_cnt != 0) {
            return false;
        }
        return true;
    }
    ...
    private function should_change_meet_class_to_in_class($token){
        if(!$this->in_php || $this->state != self::STATE_MEET_CLASS || is_array($token)){
            return false;
        }
    ...
    private function change_meet_class_to_in_class($token) {
        if($this->state != self::STATE_MEET_CLASS){
            return;
        }
        $this->state = self::STATE_IN_CLASS;
    ...
    private function change_in_func_to_init($token){
        if($this->state != self::STATE_IN_FUNC){
            return;
        }
        $this->end_line = $this->last_line;
        $this->record_information(); //记录当前的数据
    ...
    private function should_change_in_func_to_init($token) {
        if (!$this->in_php || $this->state != self::STATE_IN_FUNC || is_array($token)) {
            return false;
        }
        if (is_string($token) && $token == "}" && $this->left_func_method_brace_cnt == 1 &&
            $this->left_class_brace_cnt == 0) {
    ...
    {
        if (!$this->in_php || $this->state != self::STATE_INIT || !is_array($token) || $token[0] != T_FUNCTION) {
            return false;
        }
        if ($this->left_class_brace_cnt != 0 || $this->left_func_method_brace_cnt != 0) {
            return false;
        }
        return true;
    }
    ...
    {
        if (!$this->in_php || $this->state != self::STATE_INIT || !is_array($token) || $token[0] != T_CLASS) {
            return false;
        }
        if ($this->left_class_brace_cnt != 0 || $this->left_func_method_brace_cnt != 0) {
            return false;
    ...
    {
        if (!$this->in_php || $this->state != self::STATE_MEET_FUNC || is_array($token)) {
            return false;
        }
        if (is_string($token) && $token == ";") {
            return true;
    ...
    {
        if ($this->state != self::STATE_MEET_FUNC) {
            return;
        }
        $this->state = self::STATE_INIT;
        if($token == "{"){
